...
public class CustomLoginFailuerHandler implements AuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest req, HttpServletResponse res, AuthenticationException exception) throws IOException, ServletException {
        ...
        log.info("Failed login attempt for user {}. Reason: {} From IP: {}", userId, exception.getClass().getSimpleName(), req.getRemoteAddr());

        String exceptionMessage = messageAndException(exception);

        if(exception instanceof BadCredentialsException) {
            handleBadCredentials(res, exceptionMessage, userId);
            return;
        }
       ...
       if (exception instanceof DisabledException) {
            ...
            String message = (UserStatus.STOP.toString().equals(userStatus))
                    ? LoginErrorMessage.DisabledException_STOP.getValue() : LoginErrorMessage.DisabledException_WAIT.getValue();
    ...
    private void handleBadCredentials(HttpServletResponse res, String exceptionMessage, String userId) throws IOException, ServletException {
        int failCount = failCnt(userId);
        if (failCount < 5) {
            messageView(res, String.format("%s / 5회 실패하셨습니다. ", failCount) + "\n" + exceptionMessage);
        } else {
            messageView(res, LoginErrorMessage.LockedException.getValue());
        }
    }
