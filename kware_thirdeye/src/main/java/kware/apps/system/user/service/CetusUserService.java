...
import kware.apps.system.group.groupuser.service.CetusGroupUserService;
import kware.apps.system.position.positionuser.service.CetusPositionUserService;
import kware.apps.system.statushist.service.CetusUserStatusHistService;
import kware.apps.system.user.domain.CetusUser;
import kware.apps.system.user.domain.CetusUserDao;
import kware.apps.system.user.dto.request.*;
import kware.apps.system.user.dto.response.UserExcelPage;
import kware.apps.system.user.dto.response.UserFullInfo;
import kware.apps.system.user.dto.response.UserList;
import kware.apps.system.workplace.workplaceuser.service.CetusWorkplaceUserService;
...
public class CetusUserService {
    ...
    public void renderEXCEL(UserExcelSearch search) {
        ...
        try {
            excelCreate.createExcelFile(
                    UserExcelPage.class,
                    p -> {
                        Page<UserExcelPage> excelPage = dao.userExcelPage(search, p);
                        List<UserExcelPage> userList = excelPage.getList().stream().map(dto -> {
                            dto.setRoleNm(UserAuthorCd.getDescriptionByCode(dto.getRole()));
                            dto.setStatusNm(UserStatus.getDescriptionByCode(dto.getStatus()));
                            return dto;
                        }).collect(Collectors.toList());
                        excelPage.setList(userList);
                        return excelPage;
                    },
    ...
    public UserFullInfo findUserByUserId(String userId) {
        UserFullInfo fullInfo = dao.getUserByUserId(userId);
        fullInfo.setRoleNm(UserAuthorCd.getDescriptionByCode(fullInfo.getRole()));
        fullInfo.setStatusNm(UserStatus.getDescriptionByCode(fullInfo.getStatus()));
        return fullInfo;
    }
    ...
    public UserFullInfo findUserFullInfoByUserUid(Long uid) {
        UserFullInfo fullInfo = dao.getUserFullInfoByUserUid(uid);
        fullInfo.setRoleNm(UserAuthorCd.getDescriptionByCode(fullInfo.getRole()));
        fullInfo.setStatusNm(UserStatus.getDescriptionByCode(fullInfo.getStatus()));
        return fullInfo;
    }
    ...
    public Page<UserList> userPage(UserListSearch search, Pageable pageable) {
        search.setWorkplaceUid(UserUtil.getUserWorkplaceUid());
        Page<UserList> userPage = dao.userPage(search, pageable);
        List<UserList> userList = userPage.getList().stream().map(dto -> {
            dto.setRoleNm(UserAuthorCd.getDescriptionByCode(dto.getRole()));
            dto.setStatusNm(UserStatus.getDescriptionByCode(dto.getStatus()));
            return dto;
        }).collect(Collectors.toList());
