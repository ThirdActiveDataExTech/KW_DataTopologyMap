...
public enum DatasetMainUiType {
    ...
    TYPE_A(1, "카테고리 + 백그라운드 이미지 + 정보(제목)", true, false, true),
    TYPE_B(2, "카테고리 + 백그라운드 이미지 + 액션버튼 + 정보(제목 및 설명)", true, true, true),
    TYPE_C(3, "백그라운드 이미지 + 정보(제목 및 설명)", true, true, true),
    TYPE_D(4, "정보(제목 및 설명)", true, true, false);
    ...
    DatasetMainUiType(Integer order, String description, boolean useTitle, boolean useSummary, boolean useThumbNail) {
        this.order = order;
