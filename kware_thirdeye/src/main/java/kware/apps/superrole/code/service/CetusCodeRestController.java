package kware.apps.superrole.code.service;

import cetus.bean.Page;
import cetus.bean.Pageable;
import kware.apps.superrole.code.dto.request.*;
import kware.apps.superrole.code.dto.response.CodePage;
...
public class CetusCodeRestController {
    ...
    public ResponseEntity addCode(@RequestBody @Valid CodeSave request) {
        ...
        if (findCodeCnt > 0) return ResponseEntity.status(HttpStatus.CONFLICT).body("중복된 그룹 코드ID 입니다.");
    ...
    public ResponseEntity getChildCodes(String code, String useAt) {
        ...
        if (childCodes.isEmpty()) return ResponseEntity.noContent().build();
    ...
    public ResponseEntity codesByParentCode(@PathVariable String upperCode) {
        ...
        if (codesByParentCode.isEmpty()) return ResponseEntity.noContent().build();
